...
test('进度筛选和标记结果共用同一套状态按钮样式', () => {
  expect(getExecutionOptionClassName('danger', false)).toBe('execution-status-button danger')
...
test('执行模式隐藏左上角媒体工具栏', () => {
  expect(shouldShowMediaToolbar(false)).toBe(true)
  expect(shouldShowMediaToolbar(true)).toBe(false)
})
